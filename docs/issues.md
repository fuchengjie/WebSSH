...
- **现状**：用户确认 WebSSH 作为跳板机场景下可做。已实现：前端粘贴私钥内容，后端写入本次会话临时文件并 `addIdentity`，会话结束自动删除。
- **处理记录**：
  - ✅ 已完成。私钥内容不持久化、不进 git；JSch 0.1.55 要求 PEM 格式，OpenSSH 新格式需 `ssh-keygen -p -f keyfile -m pem` 转换。
...
## 2026-07-01 同步更新（其他 AI 工具完成）

### 连接预检修复
你反馈的「命令行 ssh 可以连，但 WebSSH 点击连接没反应」已修复，问题由两个原因叠加导致：
- 后端 `WebSSHServiceImpl.testConnect` 硬编码加载 `/Users/fu/.ssh/id_rsa`，该文件不存在时会抛异常并阻止连接测试；已删除该硬编码。
- 前端 `index.html` 的 layui form filter 与提交按钮不匹配，且 `$.post` 的同步写法在现代浏览器下行为异常；已改为显式 `$.ajax({async: false})` 并统一 filter 为 `frm`，同时增加 loading 状态和错误提示。

### UI 视觉重构
`dbf5763` 对前端页面进行了整体视觉升级：
- 登录页：`index_style.css` + 新的 `login-hero-ai.png` 背景图，提示更友好。
- 终端页：`terminal.html` 布局调整，终端区域自适应显示更完整。
- 文件管理页：`upload.html` 增加上级目录、新建目录、目录状态提示、文件排序等。

### 文件管理额外增强
- `FileUtil` 增加 `mkdir()`、文件名安全校验（禁止 `../` 和路径分隔符）、目录排在文件前面、上传直接读输入流避免中间临时文件。
- `FileController` 增加 `/file/mkdir` 和统一异常处理（`@ExceptionHandler`），错误信息直接返回前端。

### 验证
- 已用你提供的真实主机（110.40.185.118:20022 / ubuntu）在本地验证通过：连接测试成功、可进入终端页。
